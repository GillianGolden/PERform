...
to setup
  ...
  set schools-dataset gis:load-dataset "DonegalSchools.shp"
  set population-dataset gis:load-dataset "Donegal-Population.shp"
  set set-dataset gis:load-dataset "DonegalSettlements.shp"
  set cou-dataset gis:load-dataset "Donegal_county.shp"
...
to draw_ed
  ...
  gis:draw set-dataset 1
end

to draw_cou
  gis:set-drawing-color white
  gis:draw cou-dataset 1
end
...
;  [
  ask populations
   [
      ...
      if gbl_school_total > 2
      [
        ask n-of 1 other populations with [roll_no = [roll_no] of myself]
        [
          set gbl_risk_counter gbl_risk_counter + risk_score
        ]
     set friend_counter friend_counter + 1
     set friends_risk_counter friends_risk_counter + gbl_risk_counter
     if friends_risk_counter > 0
      [set average_friends_risk friends_risk_counter / friend_counter]
     set gbl_risk_counter 0

    let target other populations in-radius 2
    if count target > 4
        [
         ask n-of 1 target
         [
...
;  ]
  set risk0 mean [average_friends_risk] of populations with [risk_score = 0 AND friend_counter > 0]
  set risk1 mean [average_friends_risk] of populations with [risk_score = 1 AND friend_counter > 0]
  set risk2 mean [average_friends_risk] of populations with [risk_score = 2 AND friend_counter > 0]
  set risk3 mean [average_friends_risk] of populations with [risk_score = 3 AND friend_counter > 0]
  set risk4 mean [average_friends_risk] of populations with [risk_score = 4 AND friend_counter > 0]
  set risk5 mean [average_friends_risk] of populations with [risk_score = 5 AND friend_counter > 0]
tick
...
to do_export
  set filename word behaviorspace-run-number "export.csv"
...
"" ""
PENS
"Risk 0" 1.0 0 -4399183 true "" "plot risk0"
"Risk 1" 1.0 0 -11221820 true "" "plot risk1"
"Risk 2" 1.0 0 -7500403 true "" "plot risk2"
"Risk 3" 1.0 0 -2674135 true "" "plot risk3"
"Risk 4" 1.0 0 -955883 true "" "plot risk4"
"Risk 5" 1.0 0 -6459832 true "" "plot risk5"
...
@#$#@#$#@
<experiments>
  <experiment name="autorun" repetitions="2" runMetricsEveryStep="true">
    <setup>setup
add_schools
add_population</setup>
    <go>make_friends</go>
    <final>do_export</final>
    <timeLimit steps="150"/>
  </experiment>
</experiments>
@#$#@#$#@
